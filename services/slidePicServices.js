...
exports.addSlidePic = (req, res, next) => {
    ...
    if(params.type === "1"){
        ...
        db.base(sql, [params.id]).then(resultFileName => {
            var fileName = resultFileName.data[0].slideImg;
            if(params.slideImg === ""){
                db.updateData(
                    "cloud_music_slide_pic",
                    ["imgType"],
                    [params.imgType, params.id]).then( result => {
                ...
                fs.unlink(`public/img/slidePic/${fileName}`, err => {
                    if(err){
                        ...
                        db.updateData(
                            "cloud_music_slide_pic",
                            ["imgType", "slideImg"],
                            [params.imgType, global.uploadFileName, params.id]).then( result => {
                            if(result.data.effectedRows !== 0){
                                res.json({
                                    status: 200,
                                    errMsg: "",
        ...
        db.addData(
            ...
            [uuid.v1(), params.imgType, global.uploadFileName, moment(new Date()).format("YYYY-MM-DD HH:mm:ss")]).then( result => {
            if(result.data.effectedRows !== 0){
                res.json({
                    status: 200,
                    errMsg: "",
...
exports.getSlidePic = (req, res, next) => {
    ...
    var sqlCount = "select count(*) as count from cloud_music_slide_pic";
    var totalRow = 0;
    ...
    db.base(sql, [id]).then( resultFileName => {
        ...
        fs.unlink(`public/img/slidePic/${fileName}`, err => {
            if(err){
                ...
                db.delData("cloud_music_slide_pic", id).then( result => {
